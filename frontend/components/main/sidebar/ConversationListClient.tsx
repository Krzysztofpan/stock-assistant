...
const ConversationListClient = ({ filter }: ConversationListClientProps) => {
  ...
  return (
    <SidebarMenu className={cn(filter === "bookmarked" && "max-h-50 min-h-0 overflow-y-auto")}>
      {conversations.length ? conversations.map((conversation) => <ConversationItem conversation={conversation} key={conversation.id} />) : <p className='text-muted-foreground text-sm mt-2'>{filter == "unbookmarked" ? "Create a new" : "Pin existing"} conversation to see</p>}
      {isFetchingNextPage && (
        <>
          <SidebarMenuSkeleton />
          <SidebarMenuSkeleton />
